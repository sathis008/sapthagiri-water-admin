...
const renderSection = (title: string, rows: { label: string; value?: React.ReactNode }[]) => {
  if (rows.length === 0) {
    return null;
  }
            ...
            {documentRows.map((row) => (
              ...
              >
                ...
                <span className="flex items-center gap-3 text-sm font-semibold text-right">
                  <span>{row.fileName || 'Not uploaded'}</span>
                  ...
                  {row.fileUrl ? (
                    <button
                      ...
                      onClick={() => {
                        const viewUrl = getDocumentViewUrl(row.fileUrl);
